...
def get_hash(df, **kwargs):
    return hash_dataframes(jhu_utils.clean_jhu(df, [("Pennsylvania", "Allegheny")]))


def process_data(df, **kwargs):
  newest = jhu_utils.clean_jhu(df, [("Pennsylvania", "Allegheny")])

  ts = jhu_utils.get_cases_time_series()
  ts = ts[(ts['Admin2'] == 'Allegheny') & (ts['Province_State'] == 'Pennsylvania')]
  ts = ts.set_index('Combined_Key').transpose().iloc[10:].rename(columns=lambda v: "Total Cases")

  ds = jhu_utils.get_deaths_time_series()
  ds = ds[(ds['Admin2'] == 'Allegheny') & (ds['Province_State'] == 'Pennsylvania')]
